...
# 指定保存路径
save_directory = os.path.join(os.path.dirname(__file__), 'img')
...
def save_captcha_info(driver, save_dir, start_index=1, tips_file_name="captcha_tips.txt"):
    """
    截取验证码图片并保存为 image-1.jpg, image-2.jpg...格式，同时获取提示的文字信息并追加保存到一个指定的txt文件中。

    参数:
    driver: WebDriver 对象
    save_dir: 图片和文本保存的文件夹路径
    start_index: 从哪个编号开始保存文件，默认为 1
    tips_file_name: 保存提示文字信息的文本文件名，默认为 "captcha_tips.txt"
    """
    # 确保目录存在
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    # 获取目录下现有图片文件的编号列表，用于确定下一个文件的编号
    existing_files = sorted([f for f in os.listdir(save_dir) if f.startswith('image-') and f.endswith('.jpg')])
    ...
    try:
        ...
        print(f"图片源数据: {img_src}")
        if img_src.startswith('data:image'):
            img_base64 = img_src.split(',')[1]
            img_data = base64.b64decode(img_base64)

            img_filename = os.path.join(save_dir, f'image-{next_index:d}.jpg')
    ...
    try:
        ...
        tips_text_file = os.path.join(save_dir, tips_file_name)
        with open(tips_text_file, 'a', encoding='utf-8') as f:
            f.write(tips_text + '\n')
        print(f"验证码提示已追加保存到 '{tips_text_file}'")
            ...
            try:
                ...
                if desired_time in time_slot:
                    ...
                    if 'reserve.gif' in img_src:
                        ...
                        # ).click()
                        refresh_and_capture(driver,save_directory,1000)
